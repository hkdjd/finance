...
JOURNAL_GENERATE=$(curl -s -X POST $BASE_URL/journal-entries/generate/$CONTRACT_ID \
  ...
  -d '{"entryType": "AMORTIZATION", "description": "生成摊销会计分录"}')
...
# 4.4 验证重新生成会计分录不影响付款分录
echo "🔄 验证重新生成摊销分录不影响付款分录..."
JE_COUNT_PAYMENT_BEFORE=$(curl -s $BASE_URL/journal-entries/contract/$CONTRACT_ID | grep -o '"entryType":"PAYMENT"' | wc -l | tr -d ' ')
curl -s -X POST $BASE_URL/journal-entries/generate/$CONTRACT_ID \
  -H "Content-Type: application/json" \
  -d '{"entryType": "AMORTIZATION", "description": "重新生成摊销会计分录"}' >/dev/null
...
# 5.1 获取附件信息
echo "📎 获取合同附件信息..."
ATTACHMENT_INFO=$(curl -s -X GET $BASE_URL/contracts/$CONTRACT_ID/attachment)
if echo $ATTACHMENT_INFO | grep -q "attachment"; then
    echo "✅ 附件信息获取成功"
    if echo $ATTACHMENT_INFO | grep -q '"fileName":' && echo $ATTACHMENT_INFO | grep -q '"downloadUrl":'; then
        echo "✅ 附件信息格式正确：包含fileName和downloadUrl字段"
    else
        echo "❌ 附件信息格式不正确"
    fi
else
    echo "⚠️ 附件信息获取: $ATTACHMENT_INFO"
fi

# 5.2 测试下载链接（仅检查响应头，不实际下载）
echo "📥 测试附件下载链接..."
DOWNLOAD_RESPONSE=$(curl -s -I $BASE_URL/contracts/$CONTRACT_ID/attachment?download=true)
if echo $DOWNLOAD_RESPONSE | grep -q "Content-Disposition"; then
    echo "✅ 下载链接正常：包含Content-Disposition头"
else
    echo "⚠️ 下载链接测试: $DOWNLOAD_RESPONSE"
fi
